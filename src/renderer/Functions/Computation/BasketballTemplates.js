...
export const writeStats = async (args) => {
  await writeToFile({
    html: `<!DOCTYPE html><html lang="en"> <head> <meta charset="UTF-8"/> <meta name="viewport" content="width=device-width, initial-scale=1.0"/> <meta http-equiv="X-UA-Compatible" content="ie=edge"/> <link rel="stylesheet" type="text/css" href="stats.css"/> </head> <body> <div> <div> <div>${args.homeTeam}</div><div>TEAM STATS</div><div>${args.awayTeam}</div></div><div> <div>${args.homeTeamFG}</div><div>FG%</div><div>${args.awayTeamFG}</div></div><div> <div>${args.homeTeam3PT}</div><div>3PT%</div><div>${args.awayTeam3PT}</div></div><div> <div>${args.homeTeamFT}</div><div>Free Throws Made</div><div>${args.awayTeamFT}</div></div><div> <div>${args.homeTeamTurnovers}</div><div>Turnovers</div><div>${args.awayTeamTurnovers}</div></div><div> <div>${args.homeTeamSteals}</div><div>Steals</div><div>${args.awayTeamSteals}</div></div></div></body></html>`,

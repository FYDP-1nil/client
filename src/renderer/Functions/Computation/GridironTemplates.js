...
export const writeStats = async (args) => {
  await writeToFile({
    html: `<!DOCTYPE html><html lang="en"> <head> <meta charset="UTF-8"/> <meta name="viewport" content="width=device-width, initial-scale=1.0"/> <meta http-equiv="X-UA-Compatible" content="ie=edge"/> <link rel="stylesheet" type="text/css" href="stats.css"/> </head> <body> <div> <div> <div>${args.homeTeam}</div><div>TEAM STATS</div><div>${args.awayTeam}</div></div><div> <div>${args.homeTeamTouchdowns}</div><div>Touchdowns</div><div>${args.awayTeamTouchdowns}</div></div><div> <div>${args.homeTeamRushing}</div><div>Rushing Yards</div><div>${args.awayTeamRushing}</div></div><div> <div>${args.homeTeamPassing}</div><div>Passing Yards</div><div>${args.awayTeamPassing}</div></div><div> <div>${args.homeTeamAvgYds}</div><div>Avg Yards Per Play</div><div>${args.awayTeamAvgYds}</div></div><div> <div>${args.homeTeamTurnovers}</div><div>Turnovers</div><div>${args.awayTeamTurnovers}</div></div></div></body></html>`,
